...
% Task
In this paper I report on formalizing homotopy type of a Moebius strip and on porting a proof of an isomorphism between a trivial fiber bundle and a dependent type using proof assistant Arend.

% Verification of different fields of math. Different proof-assistants
Proof assistants are computer systems, which are designed to do mathematics on a computer. They allow a user to define some statements and to reason about their correctness. Some of them also help user with a set of well chosen decision procedures that allow formulas of a specific restricted format to be proved automatically\autocite{ProofAssistants1}.

% Math fields
The following work was performed with one of the flavors of intensional type theory---homotopy type theory, which takes seriously the natural interpretation of identity types as formalizing path space objects in homotopy theory. This flavor comes with a few benefits: higher inductive types, which can be used to obtain quotients objects or free structures, univalence that simplify the process of moving back and forth between isomorphic structures, and a synthetic approach to homotopy theory.

% Well developed/proof-assistants/checkers under development
Theorem provers based on type theory come with different sets of most basic operations. Some of these sets are studied better than another. Arend provides its users with an interval type, an eliminator for an identity type---\textit{coe}, a dependent path type, and a function \textit{iso}, which can be used to define univalence.

% Dependent types (bare definitions with refs)
...
% Fibre bundle (bare definitions with refs)
Fiber bundles and corresponding structure can be defined as follows\autocite{nlab}:
\begin{mydefinition}[Bundle]
A bundle over an object $B$ is an object $E$ equipped with a morphism $\phi$ from $E$ to $B$:
\[
\begin{diagram}
