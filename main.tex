...
% Task
In this paper I report on formalizing homotopy type of a Moebius strip and on porting proof of isomorphism between a trivial fiber bundle and a dependent type using proof assistant Arend.
...
Theorem provers based on type theory come with different sets of most basic operations. Some of these sets are studied better than another. Arend provides its users with an interval type, an eliminator for an identity type---textit{coe}, and a function \textit{iso}, which can be used to define univalence.
...
% Example (pic)
Two fiber bundles over a circle  with fiber given by the unit interval $[0,1]$ are the cylinder and the Moebius strip. Locally they are the same, but globally the Moebius strip comes with a twist.
\begin{figure}[H]
\centering
\begin{tikzpicture}[scale=1.25]
\begin{axis}[
